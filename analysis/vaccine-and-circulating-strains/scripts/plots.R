...
#           "phangorn", "showtext", "mutSignatures", "vcfR", "kableExtra", "paletteer")
# ipak(pkg = pkgs)
# 
...
# output directories
outDir <- file.path(baseDir, "output-dir-agbt")
...
colors <- c("coral2", "chocolate3", "hotpink4", "cadetblue3", 
            "chartreuse4", "lavenderblush2",  "darkseagreen1",
            "goldenrod3", "violet", "peachpuff1", "paleturquoise3",
            "firebrick3", "darksalmon", "honeydew3", "deepskyblue1")
...
plot_tree <- function(tree, metadata, cols, strain_type_custom_colors, 
                      tips_to_label, subtitle){
  #'
  #'
  #'
  #'
  ...
  p <- 
    ...
    geom_tippoint(aes(color = strain_type), alpha = 1.0, size=8) +
    scale_color_manual(name = "Strain Type",
                       values = strain_type_custom_colors,
                       breaks = sort(unique(meta$strain_type))) +
...
plot_tree_snps <- function(snps_data, tree, metadata, column, cols){
  column <- ensym(column)
  p <- tree + geom_facet(panel = "SNPs", 
                         data = snps_data, geom = geom_point,
                         mapping=aes(x = pos, colour = !!column), 
                         size=4.5, shape = "•") +
    xlab("             subs/site/year                                          position (bp)\n") +
    # xlab("          subs/site/year                                                   position (bp)\n") +
    scale_colour_manual(name=column,
                       values=cols,
...
snps <- file.path(baseDir, "output-dir/riftL/alignment/riftL_masked_alignment_snps.csv")
...
n_colors <- length(unique(sort(metaL$strain_type)))
strain_type_custom_colors <- rev(paletteer_d("NatParksPalettes::LakeNakuru")[1:n_colors])

tips_to_label <- metaL[metaL$strain_type == 'vaccine',]$taxa
print(tips_to_label)

# read IQTREE treefile
# subset = (label %in% vac_strains)
iqtreeLarge <- read.tree(treefile)
iqtreeLarge <- phangorn::midpoint(iqtreeLarge)
plot.large.iqtree <- plot_tree(tree = iqtreeLarge, 
                               metadata = metaL,
                               cols = cols,
                               strain_type_custom_colors = strain_type_custom_colors,
                               tips_to_label = tips_to_label,
...
snp_data <- snps_data(snps_fn = snps)

tree.snps.plot <- plot_tree_snps(snps_data = snp_data,
                                 tree = plot.large.iqtree,
                                 metadata = metaL,
                                 column = "strain_type",
                                 cols = strain_type_custom_colors)
tree.snps.plot



ggsave("ML-iqtree-large.pdf", tree.snps.plot, 
...
snps <- file.path(baseDir, "output-dir/riftM/alignment/riftM_masked_alignment_snps.csv")

metaM <- prepare_metadata(metadata = metadata)
cols <- df[df$lineage %in% sort(unique(metaM$lineage)),]$color
n_colors <- length(unique(sort(metaM$strain_type)))
strain_type_custom_colors <- paletteer_d("ggsci::alternating_igv")[1:n_colors]
...
# read IQTREE treefile
iqtreeMedium <- read.tree(treefile)
iqtreeMedium <- phangorn::midpoint(iqtreeMedium)
plot.medium.iqtree <- plot_tree(tree = iqtreeMedium, 
                                metadata = metaM,
                                cols = cols,
                                strain_type_custom_colors = strain_type_custom_colors,
                                subtitle = "Glycoproteins (NSm/Gn/Gc)"
...
ggsave("ML-iqtree-medium.pdf", tree.snps.plot, 
...
snps <- file.path(baseDir, "output-dir/riftS-NSS/alignment/riftS-NSS_masked_alignment_snps.csv")

metaS <- prepare_metadata(metadata = metadata)
cols <- df[df$lineage %in% sort(unique(metaS$lineage)),]$color
n_colors <- length(unique(sort(metaS$strain_type)))
strain_type_custom_colors <- paletteer_d("ggsci::alternating_igv")[1:n_colors]
...
# read IQTREE treefile
iqtreeNSS <- read.tree(treefile)
iqtreeNSS <- phangorn::midpoint(iqtreeNSS)
plot.nss.iqtree <- plot_tree(tree = iqtreeNSS, 
                             metadata = metaS,
                             cols = cols,
                             strain_type_custom_colors = strain_type_custom_colors,
                             subtitle = "Non-structural (NSs)"
...
ggsave("ML-iqtree-nss.pdf", tree.snps.plot, 
       width = 25, height = 15, units = "in", 
       limitsize = FALSE,
       dpi = 300, bg="white", device = cairo_pdf)
...
snps <- file.path(baseDir, "output-dir/riftS-NP/alignment/riftS-NP_masked_alignment_snps.csv")
...
# read IQTREE treefile
iqtreeNP <- read.tree(treefile)
iqtreeNP <- phangorn::midpoint(iqtreeNP)
...
ggsave("ML-iqtree-np.pdf", tree.snps.plot, 
       width = 25, height = 15, units = "in", 
       limitsize = FALSE,
       dpi = 300, bg="white", device = cairo_pdf)
