...
process PREPARE_GEOLOCATIONS {
    ...
    tuple val(meta), val(csv)

    output:
    tuple val(meta), path("*_geolocations.csv"), emit: geolocations
    tuple val(meta), path("*_dates.csv"),        emit: dates

    script:
    def prefix    = options.suffix ? "${meta.id}${options.suffix}" : "${meta.id}"

    """
    prepare_geolocations.py \\
        --metadata $metadata \\
        --locations $csv \\
        --prefix ${prefix} \\
        --outDir .
