...
def referencesList = ["NC_014395", "NC_014396", "NC_014397"]
...
include { REVERSE_COMPLEMENT as REVERSE_COMPLEMENT_NP}     from '../modules/local/process/reverse_complement' addParams( options: modules['reverse_complement']                            )



include { STRAIN_TYPES  as STRAIN_TYPES_REV}     from '../modules/local/process/strain_types' addParams( options: modules['strain_types_rev']                            )
include { EXTRACT_ACCESSIONS as EXTRACT_ACCESSIONS_REV }     from '../modules/local/process/extract_accessions' addParams( options: modules['extract_accessions_rev']                            )
include { GET_SNPS_RECORDS as GET_SNPS_RECORDS_REV }     from '../modules/local/process/get_snps_records' addParams( options: modules['get_snps_records_rev']                            )
include { SNPSITES as SNPSITES_REV }     from '../modules/local/process/snpsites' addParams( options: modules['snp_sites_rev']                            )
include { EXTRACT_ABUNDANT_SNPS as EXTRACT_ABUNDANT_SNPS_REV }     from '../modules/local/process/extract_abundant_snps' addParams( options: modules['extract_abundant_snps_rev']                            )
...
workflow RVFVMUTATIONALPROFILING {
    ...
    if ( params.segment == 'S-NP' ) {
        ...
        GET_SNPS_RECORDS(EXTRACT_ACCESSIONS.out.fasta, STRAIN_TYPES.out.txt, ch_vaccine_reference, ch_grouping_column, ch_lineages)
        EXTRACT_ABUNDANT_SNPS(SNPSITES.out.vcf, ch_vaccine_reference)

        // 
        ch_reverse_complement_fasta = REVERSE_COMPLEMENT_NP(ch_prefix, FILTER_TAXA.out.fasta)
        STRAIN_TYPES_REV(ch_reverse_complement_fasta, ch_metadata, ch_lineages)
        EXTRACT_ACCESSIONS_REV(ch_reverse_complement_fasta, ch_vaccine_reference, STRAIN_TYPES_REV.out.txt, ch_grouping_column)
        SNPSITES_REV(EXTRACT_ACCESSIONS_REV.out.fasta, ch_grouping_column, ch_vaccine_reference)
        GET_SNPS_RECORDS_REV(EXTRACT_ACCESSIONS_REV.out.fasta, STRAIN_TYPES_REV.out.txt, ch_vaccine_reference, ch_grouping_column, ch_lineages)
        EXTRACT_ABUNDANT_SNPS_REV(SNPSITES_REV.out.vcf, ch_vaccine_reference)
        ...
        // 
        ch_reverse_complement_fasta = REVERSE_COMPLEMENT(ch_prefix, ch_filtered_fasta)
        STRAIN_TYPES_REV(ch_reverse_complement_fasta, ch_metadata, ch_lineages)
        EXTRACT_ACCESSIONS_REV(ch_reverse_complement_fasta, ch_vaccine_reference, STRAIN_TYPES_REV.out.txt, ch_grouping_column)
        SNPSITES_REV(EXTRACT_ACCESSIONS_REV.out.fasta, ch_grouping_column, ch_vaccine_reference)
        GET_SNPS_RECORDS_REV(EXTRACT_ACCESSIONS_REV.out.fasta, STRAIN_TYPES_REV.out.txt, ch_vaccine_reference, ch_grouping_column, ch_lineages)
